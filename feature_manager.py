...
class feature_manager(object):
    ...
    def __init__(self, method='intuitive', tolerance=0.75, ratio_num_range=(15, 50)):
        ...
        self.observations = {'train':None}  ### Dictionary of the observation indices for each dataset trained/tested
        ...
        self.trfm = {'whiten':None,'pca':None,'lasso':None} ### Whitening & PCA & LASSO object used in transformation
    ...
    def restrict(self, data):
        ...
        if self.method not in ('ohlson','intuitive','all_ratios'):
            self.ratio_num_range = (max(self.ratio_num_range[0],1), min(self.ratio_num_range[1],data.shape[1]-2))
            if self.method == 'pca':
                ...
                data = self.lasso_restrict(data, a=1e-1, rnr=(self.ratio_num_range[1], data.shape[1]-2))    # No INF & NAN
                data = self.pca_restrict(data, min_var=-1)

            elif self.method == 'pca_lasso':  ### Do PCA first, then Lasso
                data = self.pca_restrict(data, min_var=0.95, rnr=(self.ratio_num_range[1], data.shape[1]-2)) # No INF & NAN
    ...
    def pca_restrict(self, data, curvature=True, rnr = None, show_plots = False):
        ...
        if curvature:
            ...
            n_components = max(np.argmin(d2c_h4[3:])+7, np.argmin(d1v_h4[3:])+7, sum(c<self.TOL)+1, rnr[0])
            ...
            if show_plots:
                ...
                ax1.set_title('Second derivative of explained varaince wrt included components')
        ...
        if n_components<rnr[0]: 
            print('PCA chose too few components, used range minimum number of variables(%d) instead of %d'%(rnr[0],n_components))
            n_components = rnr[1]
        if n_components>rnr[1]: 
            print('PCA chose too many components, used range maximum number of variables(%d) instead of %d'%(rnr[1],n_components))
            n_components = rnr[1]
        ...
        data =data.loc[:,self.flag]#data.drop(self.features.values, axis=1, inplace=True)  # No INF & NAN
    ...
    def lasso_restrict(self, data, a=0.01,  rnr = None):
        ...
        smoothed = pd.DataFrame([self.lasso_smooth(data[i],data[self.flag]) for i in data.columns[1:]]).T
